...
/// silently no-ops the transfer table there (§21‑E1). This stub classifies as
...
public struct GammaBackend: WarmthBackend {
    ...
    public func classify(_ identity: DisplayIdentity) async -> Capability<Void> {
        // TODO(milestone): real classification by device/OS build. On M5 Tahoe the gamma
    ...
    public func apply(_ kelvin: Kelvin, to identity: DisplayIdentity) async throws {
        // TODO(milestone): build per-channel ramps from rgbGain(for:) and push them via
    ...
    public func reset(_ identity: DisplayIdentity) async throws {
        // TODO(milestone): CGDisplayRestoreColorSyncSettings() to drop any applied ramp.
    }
}
