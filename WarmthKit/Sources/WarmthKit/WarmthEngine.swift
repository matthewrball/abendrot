...
public actor WarmthEngine {
    ...
    /// so warmth / DDC opt-in / layer override survive an unplug→replug (stable identity,
    /// re-applies per-display state). Keyed by `persistentKey`.
    ...
    /// living in the engine is the contract; the observers themselves are dumb emitters.
    ...
    /// native, BEFORE this run applies anything (invariant 7). Each recovered key is drained
    ...
    /// Test seam: drive a hotplug/wake reconfiguration with an explicit display set.
            ...
            if effective == .off {
                ...
                if layer == .hardware {
                    ...
                    do {
                        ...
                        // the Hardware badge (invariant 1 + honest badges). The dirty flag stays
    ...
    /// mirroring the apply branch, so the "always recoverable" invariant (invariant 7) holds
    ...
    private func recommend(
        ...
        // becomes the badge once the user enables it for a display.
