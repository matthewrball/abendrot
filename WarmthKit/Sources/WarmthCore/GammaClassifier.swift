...
///
/// The hard-won fact this encodes (refined by the §25 hardware test): the gamma transfer table
...
public enum GammaClassifier {
    ...
    /// (§25; see the code-review HIGH finding on brand-string robustness.)
    ...
    public static func classify(_ environment: Environment) -> Capability<Void> {
        ...
        // M-series round-trips correctly (verified on hardware, §25), so it is NOT denylisted.
