...
# vocabulary (Mode A/B, Wave-N, Lane X, dev/dogfood, "Session N"/"S1N" dev-cadence refs), the
# internal RELEASE.md/abendrot-plan paths, the handoff/resume artifacts, "founder", absolute home
# paths (/Users, /home), and the private repo names (abendrot-build/-public). Any surviving hit
# fails the run.
GATE_PATHS=()
for t in "${SYNC_TREES[@]}"; do [ -d "$PUBLIC/$t" ] && GATE_PATHS+=("$PUBLIC/$t"); done
for f in "${SYNC_FILES[@]}"; do [ -f "$PUBLIC/$f" ] && GATE_PATHS+=("$PUBLIC/$f"); done

TELL_PATTERN='§|docs/(research|marketing|engine|qa|release)/|plan §|abendrot-plan|RESUME-PROMPT|HANDOFF\b|\bfounder\b|\bMode [AB]\b|\bmode [AB]\b|\bWave-[0-9]|\bLane [A-Z]\b|dogfood|RELEASE\.md|/Users/|/home/[a-z]|abendrot-(build|public)|\bSession [0-9]|\bS1[0-9]\b'
